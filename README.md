...
- **视图缩放**：顶部两个滑杆 —— 「波形」调缩略图大小（水平方向，越大越清楚）、「行间距」调条目疏密（垂直方向，0–30px，0=最紧凑）；设置自动记住（localStorage）
- **目录浏览**：右上「📁 目录」面板（资源管理器式）——打开自动展开并定位到当前目录；目录树 ⭐ 收藏常用目录；面包屑每段悬停浮现 ▾、点名字跳转、点 ▾ 换同级目录；选中目录后面板保持展开可连续钻；列表进入浏览模式（分页加载 + 排序：默认/路径/大小/时间/随机），搜索框自动变为「在目录内搜索」
