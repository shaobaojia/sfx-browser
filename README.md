...
## 功能
- **打字即搜**：文件名 + 路径；中文 / 英文 / 中英混合。内置「中英桥」同义词（耳鸣→tinnitus/ringing）和中文复合词拆解（金属撞击 → metal/impact…），毫秒级返回；右侧「格式」下拉可筛 WAV / MP3 / WMA / AIFF（aif+aiff 合并；全库搜索与目录内通用）
...
- **视图缩放**：顶部两个滑杆 —— 「波形」调缩略图大小（水平方向，越大越清楚）、「行间距」调条目疏密（垂直方向，0–30px，0=最紧凑）；设置自动记住（服务端，跨设备生效）
- **目录浏览**：右上「📁 目录」面板（资源管理器式）——打开自动展开并定位到当前目录；目录树 ⭐ 收藏常用目录；面包屑：点段名跳层、点「›」列出下一层目录（当前所在项加粗，直接横跳/回跳）、路尾「▾」继续下钻当前目录；选中目录后面板保持展开可连续钻；列表进入浏览模式（分页加载 + 排序：默认/路径/大小/时间/随机），搜索框自动变为「在目录内搜索」
...
- `smoke_test.py` — 端到端冒烟测试（协议级，34 项断言）
- `data/sfx.db` — 索引（~52MB，git 排除）；`cache/` — 波形与转码缓存（可随时删）
- `data/user.db` — 用户状态小库（视图/开关/导出目录/收藏/篮子；零 localStorage，旧版本地值首次打开自动迁移上来）
